      ...
      - name: Generate PR Comment
        ...
        with:
          script: |
            ...
            |-------|--------|----------|---------|
            | 🏗️ Build | ${buildStatus} | ${buildDuration} | [Logs](${buildUrl}) |
            | 🧪 Tests | ${testStatus} | ${testDuration} | ${testsPassed}/${testsTotal} passed |
            | 📊 Coverage | ${coverageStatus} | - | ${coverage}% (${coverageDiff}) |
            | 🔒 Security | ${securityStatus} | ${securityDuration} | ${vulnerabilities} issues |
            | 📝 Lint | ${lintStatus} | ${lintDuration} | ${lintIssues} warnings |
...
class AlertAggregator {
  ...
  private generateAlertSummary(alerts: Alert[]): NotificationSummary {
    return {
      title: `🔔 ${alerts.length} similar alerts in ${alerts[0].service}`,
  ...
  private async handleDeploy(command: SlashCommand): Promise<void> {
    ...
    if (!(await this.hasDeployPermission(command.user_id, environment))) {
      return this.respond(command.response_url, {
        text: '❌ You do not have permission to deploy to ' + environment,
      });
    }
