...
export class TradingDomainService {
  ...
  calculateBreakeven(positions: Array<{ price: number; quantity: number }>): number {
  ...
  calculateProfitTarget(entryPrice: number, riskRewardRatio: number, stopLoss: number): number {
    const risk = Math.abs(entryPrice - stopLoss);
    const reward = risk * riskRewardRatio;

    return stopLoss < entryPrice ? entryPrice + reward : entryPrice - reward;
  }
